...
for word in vocabulary:
    mu_spam = (df_spam[word].sum() + alpha) / (n_spam + alpha * n_vocabulary)
    sigma_spam = math.sqrt((pow((df_spam[word] - mu_spam), 2)).sum() / n_spam)
    p_word_given_spam[word].append(mu_spam)
    p_word_given_spam[word].append(sigma_spam)
    mu_ham = (df_ham[word].sum() + alpha) / (n_ham + alpha * n_vocabulary)
    sigma_ham = math.sqrt((pow((df_ham[word] - mu_ham), 2)).sum() / n_ham)
    p_word_given_ham[word].append(mu_ham)
    p_word_given_ham[word].append(sigma_ham)

# Dự đoán một thư mới
half_log_e = math.log(math.exp(1)) / 2
sqrt_2pi = math.sqrt(2 * math.pi)


def log_p_gaussian(x, mu, sigma):
    return -(pow((x - mu) / sigma, 2) * half_log_e + math.log(sigma) + sqrt_2pi)
...
def predict(doc):
    ...
    vocabulary_tmp = {}
    for word in doc:
        if word in vocabulary_tmp.keys():
            vocabulary_tmp[word] += 1
        else:
            vocabulary_tmp[word] = 1

    for word in vocabulary_tmp:
        n_word = vocabulary_tmp[word]
        if word in vocabulary:
            p_spam_given_words += log_p_gaussian(n_word, p_word_given_spam[word][0], p_word_given_spam[word][1])
            p_ham_given_words += log_p_gaussian(n_word, p_word_given_ham[word][0], p_word_given_ham[word][1])
